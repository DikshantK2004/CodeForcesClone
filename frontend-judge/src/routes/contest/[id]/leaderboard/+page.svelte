		...
		<thead>
			<tr>
				...
				{#each Array.from({ length: num_problems }, (_, i) => i + 1) as problemNum}
					<th class="prob"
						><a href="/contest/{data.contest.id}/problems/{problemNum}">{problemNum}</a></th
					>
				{/each}
			</tr>
		</thead>
				...
				<tr>
					...
					{#each Array.from({ length: num_problems }, (_, i) => i + 1) as problemNum}
						{#if entry.data[problemNum]}
							<td
								class="prob"
								class:verdict-green={entry.data[problemNum].accept}
								class:verdict-red={!entry.data[problemNum].accept}
							>
								<!-- svelte-ignore a11y-click-events-have-key-events -->
								<!-- svelte-ignore a11y-no-static-element-interactions -->
								<span
									on:click={() => {
										showDialog(entry.data[problemNum].submissions);
									}}
									>{entry.data[problemNum].accept ? '+' : '-'}{entry.data[problemNum].submissions
										.length}</span
								>
							</td>
						{:else}
